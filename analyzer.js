...
class SignalAnalyzer {
    async analyzePair(pair, timeframe = '5m') {
        try {
            const ohlcv = await priceFetcher.fetchOHLCV(pair.symbol, timeframe, 100);
            if (!ohlcv || ohlcv.length < 20) return this.fallbackSignal(pair.name, timeframe);

            const closes = ohlcv.map(c => c.close);
            const highs = ohlcv.map(c => c.high);
            const lows = ohlcv.map(c => c.low);

            // Fast indicators (small period)
            ...
            if (!rsi.length || !macd.length) return this.fallbackSignal(pair.name, timeframe);
            ...
            else if (cur.rsi < 45) { score += 20; reasons.push(`RSI low (${cur.rsi.toFixed(1)})`); direction = 'CALL'; }
            else if (cur.rsi > 55) { score += 20; reasons.push(`RSI high (${cur.rsi.toFixed(1)})`); direction = 'PUT'; }
            ...
            // Stochastic
            if (cur.stochK < 20) { score += 10; reasons.push('Stoch oversold'); if(direction !== 'PUT') direction = 'CALL'; }
            else if (cur.stochK > 80) { score += 10; reasons.push('Stoch overbought'); if(direction !== 'CALL') direction = 'PUT'; }

            if (direction === 'NEUTRAL') direction = cur.rsi < 50 ? 'CALL' : 'PUT';
            let confidence = Math.min(98, Math.max(70, 60 + Math.floor(score / 2)));
            if (direction === 'CALL' && cur.rsi > 70) confidence -= 15;
            if (direction === 'PUT' && cur.rsi < 30) confidence -= 15;
            confidence = Math.max(70, Math.min(98, confidence));

            // Last 30 candles for chart
            const lastCandles = ohlcv.slice(-30);
            const closesLast = lastCandles.map(c => c.close);
            const ema9v = this.calcEMA(closesLast, 9);
            const ema21v = this.calcEMA(closesLast, 21);
            ...
            return this.fallbackSignal(pair.name, timeframe);
        }
    }
    ...
    fallbackSignal(pair, timeframe) {
        ...
        return {
            ...
            confidence,
            reasons: ['Technical signal', 'Trend momentum', 'Entry confirmed'],
            rsi: isCall ? 35 : 65,
            adx: 28,
            timeframe,
            candles,
