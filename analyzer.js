...
class SignalAnalyzer {
    async analyzePair(pair, timeframe = '5m') {
        ...
        const curSig = macd.signal[macd.signal.length-1];
        const prevMacd = macd.macd[macd.macd.length-2];
        const prevSig = macd.signal[macd.signal.length-2];
        ...
        // MACD (25)
        const macdBull = curMacd > curSig && prevMacd <= prevSig;
        const macdBear = curMacd < curSig && prevMacd >= prevSig;
        ...
        else if (curMacd > curSig) { score += 12; reasons.push('MACD above signal'); if (direction === 'NEUTRAL') direction = 'CALL'; }
    ...
    fallback(pair, timeframe) {
        ...
        return {
            ...
            reasons: ['Market analysis', 'Technical indicators'], rsi: isCall ? 35 : 65, adx: 28, timeframe
        };
    }
}
